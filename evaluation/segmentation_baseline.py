...
def prepare_xmem_tracking(scene_dir, mask, object_id, frame_id):
    process_dir = os.path.join(scene_dir, 'segmentation_baseline')
...
def bundleprep(scene_dir, xmem_dir, output_path):
    ...
    print(f"Copying frames to bundlesdf input folder {bundlesdf_input_dir}.")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(config.scene_dir, 'segmentation_baseline')):
        os.makedirs(os.path.join(config.scene_dir, 'segmentation_baseline'), exist_ok=True)
    ...
    # Segment with SAM
    image_path = os.path.join(args.scene_dir, "rgb", f"{config.segmentation_frame}.png")
    ...
    imsave(os.path.join(config.scene_dir, 'segmentation_baseline', f"best_sam_mask_obj{config.object_id}_{config.segmentation_frame}.png"),
    ...
    # xmem_dir = '/local/home/vincentv/code/motion_segment2/data/data0101_1404/segmentation_baseline/xmem/xmem_input_data0101_1404_object1'


    # # Visualize XMem tracking results
    create_overlay_gif(scene_dir=config.scene_dir,
                       mask_dir=os.path.join(xmem_dir, 'video1'),
                       output_path=os.path.join(config.scene_dir, 'segmentation_baseline', f'overlay_global{config.object_id}.gif'))


    # Bundleprep
    dataset_name = config.scene_dir.split('/')[-1]
    bundleprep(scene_dir=config.scene_dir, xmem_dir=xmem_dir,
               output_path=os.path.join(config.scene_dir, 'segmentation_baseline',f'bundlesdf_input_{dataset_name}_segbaseline_object{config.object_id}'))
