...
from new_utils.gradslam_datasets import *
...
def interaction_detection_plot(distances_fingertip_static,
    ...
    plot_with_offset(distances_fingertip_static, arm_entry_index, plt.gca())
    plot_with_offset(distances_fingertip_object, arm_entry_index, plt.gca())
    ...
    for i, segment in enumerate(segments):
        ...
        # Bands
        plt.axvspan(segment['period'][0], segment['period'][1], facecolor=band_color, alpha=0.5)

        # Text labels
        text_coordinates = [(segment['period'][0] + segment['period'][1]) / 2, plt.gca().get_ylim()[0] + y_range * y_position_multiplier]
        plt.text(*text_coordinates, f"interaction {i}", ha='center', fontsize=11.0)

        # Arrow markers
        marker_coordinates = text_coordinates
        marker_coordinates[1] -= y_range * 0.03
        plt.plot(*marker_coordinates, marker=r'$\downarrow$', color='black')

    plt.legend(["$d(hand, initial)$", "$d(hand, object)$"])
    ...
    # ax2.legend([r"$\Delta = d(hand, initial) - d(hand, object)$"], loc="lower right")
    ...
    # ax_title.set_xticks([])
    # ax_title.set_yticks([])
    plt.gca().set_title("Distances for interaction separation", fontsize=16)


def separate_tracks(scene_dir, config):
    ...
    segments = [s for s in segments if s["period"][1] - s["period"][0] > config.track_separation["minimum_track_length"]]
