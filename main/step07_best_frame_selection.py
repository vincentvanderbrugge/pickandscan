...
from new_utils.utils import create_masked_frame
...
def select_best_frames(scene_dir, config):

    iou_threshold = config.best_frame_selection["iou_threshold"]
    ...
    for track_id, track in enumerate(tracks):
        ...
        for frame_id in tqdm(range(track["period"][0], track["period"][1])):
        ...
        # Split into above-threshold-iou periods
        max_period, periods = longest_true_period(iou_array > iou_threshold)

        # Select best frame index for this track
        if len(periods) == 0:
            print("Warning: track {} has no periods with IoU above threshold".format(track_id))
            selected_index = int((track["period"][0] + track["period"][1]) / 2)
            no_above_threshold_iou_period = True
        else:
            ...
            no_above_threshold_iou_period = False

        # Save mask overlay of selected best frame
        ...
        imsave(os.path.join(vis_dir, f"object{track_id}_frame{selected_index}.png"), visualization.astype(np.uint8),
               check_contrast=False)

        # Save best frame selection visualization
        ...
        tracks[track_id]["above_threshold_iou_periods_exist"] = not no_above_threshold_iou_period

    # Save best frame selection for all tracks in config.yaml
    # (discarding ones without above-threshold-iou period)
    tracks = [track for track in tracks if track["above_threshold_iou_periods_exist"]]
