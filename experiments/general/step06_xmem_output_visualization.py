...
if __name__ == '__main__':
    ...
    vis_dir = os.path.join(scene_dir, 'process_dir', 'xmem_output_visualizations')
    ...
    for frame_id in tqdm(range(args.start_frame, n_frames, args.nth_frame)):
        ...
        # mask = None
        mask = load_image(
            os.path.join(scene_dir, 'process_dir', 'xmem', 'xmem_output', 'video1', '%05d.png' % frame_id))
    ...
    animation_fig = animation.FuncAnimation(fig, update, frames=n_frames//args.nth_frame, interval=int(30*args.nth_frame*args.slow_down), blit=True,
                                            repeat_delay=10, )
    animation_fig.save(os.path.join(scene_dir, 'process_dir', 'xmem_output_visualizations', 'xmem_output.gif'))
